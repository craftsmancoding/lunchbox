...
class License {
    ...
    public $product_url = 'https://craftsmancoding.com/products/downloads/lunchbox/';
    public $plugin = 'Lunchbox'; // item name from store
    public $setting = 'lunchbox.license_key';
    ...
    public function activate($license_key) {
        ...
        {
            $this->modx->log(\xPDO::LOG_LEVEL_INFO, 'Activated Lunchbox License:' .print_r($response,true));
    ...
    public function check($license_key) {
        ...
        {
            $this->modx->log(\xPDO::LOG_LEVEL_INFO, 'Lunchbox -- checking remote server for license status: '. print_r($response,true));
            // We append the license key to the response to store it for comparison later
            $response['key'] = $license_key;
            $this->modx->cacheManager->set($this->setting.$license_key, $response, 60*60*24*$this->license_check_frequency_in_days, $this->cacheOptions);
