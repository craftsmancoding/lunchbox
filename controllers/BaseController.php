...
class BaseController extends \modExtraManagerController {
   ...
   // public $scriptProperties = array();
    ...
    public function fetchTemplate($file) {
        ...
        $path = $this->modx->getOption('lunchbox.core_path','', MODX_CORE_PATH.'components/lunchbox/').'views/';
    ...
    public function setMsg($msg,$type='success') {
        ...
        $path = $this->modx->getOption('lunchbox.core_path','', MODX_CORE_PATH.'components/lunchbox/').'views/msgs/';
        $file = $path.$type.'.php';
        if (is_file($file)) {
            ob_start();
            include $file;
